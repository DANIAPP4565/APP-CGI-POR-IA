...
def mostrar_metricas_hemodinamicas_digitalizacion(df_der: pd.DataFrame, puntos: Dict[str, Any]) -> None:
    ...
    try:
        dom_ct = construir_dominios_hemodinamicos(df_der, puntos)
        st.info("Conclusión del dominio contractilidad: " + str(dom_ct.get("contractilidad", "No clasificable")))
    except Exception:
        pass
...
# ============================================================
# APRENDIZAJE SUPERVISADO PERSISTENTE DE CURSORES QRS-B-C-X
# ============================================================

APRENDIZAJE_DIR = Path(os.environ.get("ICG_LEARNING_DIR", ".icg_learning"))
APRENDIZAJE_DIR.mkdir(parents=True, exist_ok=True)
APRENDIZAJE_JSON = APRENDIZAJE_DIR / "aprendizaje_cursores_qbcx.json"
APRENDIZAJE_XLSX_NOMBRE = "aprendizaje_delta_cursores_qbcx.xlsx"


def estado_aprendizaje_vacio() -> Dict[str, Any]:
    return {
        "version": "2.0-supervisado-persistente",
        "n": 0,
        "error_ms_acum": 0.0,
        "error_ms_prom": None,
...
def cargar_aprendizaje_persistente() -> Dict[str, Any]:
    """Carga el historial de aprendizaje desde disco para conservarlo entre sesiones Streamlit."""
    try:
        if APRENDIZAJE_JSON.exists():
            with open(APRENDIZAJE_JSON, "r", encoding="utf-8") as f:
                data = json.load(f)
            base = estado_aprendizaje_vacio()
            base.update(data if isinstance(data, dict) else {})
            return base
    except Exception:
        pass
    return estado_aprendizaje_vacio()


def guardar_aprendizaje_persistente(estado: Dict[str, Any]) -> None:
    """Guarda el aprendizaje validado por el médico de forma persistente."""
    try:
        estado = estado or estado_aprendizaje_vacio()
        estado["ultima_actualizacion"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        tmp = APRENDIZAJE_JSON.with_suffix(".json.tmp")
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(estado, f, ensure_ascii=False, indent=2)
        os.replace(tmp, APRENDIZAJE_JSON)
    except Exception:
        # No bloquear el flujo clínico por un problema de escritura local.
        pass


def inicializar_aprendizaje_en_session() -> None:
    if "aprendizaje_cursores" not in st.session_state:
        st.session_state["aprendizaje_cursores"] = cargar_aprendizaje_persistente()


def hash_curva_operativo(df_curva: Optional[pd.DataFrame]) -> str:
    """Huella no identificatoria de la morfología para auditoría del aprendizaje."""
    try:
        ...
        t = df_curva["tiempo_ms"].to_numpy(dtype=float)
        arr = np.column_stack([
            np.linspace(float(np.nanmin(t)), float(np.nanmax(t)), 64),
            np.interp(np.linspace(float(np.nanmin(t)), float(np.nanmax(t)), 64), t, y),
        ])
...
def features_curva_qbcx(df_curva: Optional[pd.DataFrame], puntos: Optional[Dict[str, Any]] = None, fc: Optional[float] = None) -> Dict[str, float]:
    """Extrae rasgos simples de la curva y de la propuesta automática para comparar casos similares."""
    out: Dict[str, float] = {}
    try:
        if df_curva is None or df_curva.empty:
            return out
        t = df_curva["tiempo_ms"].to_numpy(dtype=float)
        y = df_curva["amplitud_relativa"].to_numpy(dtype=float)
        dur = float(np.nanmax(t) - np.nanmin(t)) if len(t) > 1 else np.nan
        y0 = y - np.nanmedian(y)
        out.update({
            "duracion_ms": dur,
            "fc": float(fc) if fc is not None and np.isfinite(float(fc)) else np.nan,
            "amp_rango": float(np.nanmax(y0) - np.nanmin(y0)),
            "amp_std": float(np.nanstd(y0)),
            "asimetria_pico_valle": float((abs(np.nanmax(y0)) + 1e-9) / (abs(np.nanmin(y0)) + 1e-9)),
        })
        if puntos:
            for k in ["q_ms", "b_ms", "c_ms", "x_ms", "pep_ms", "lvet_ms", "bc_ms", "cx_ms"]:
                if puntos.get(k) is not None:
                    out[k] = float(puntos.get(k))
            if dur and np.isfinite(dur) and dur > 0:
                out["b_rel"] = float(puntos.get("b_ms", np.nan)) / dur
                out["c_rel"] = float(puntos.get("c_ms", np.nan)) / dur
                out["x_rel"] = float(puntos.get("x_ms", np.nan)) / dur
        return out
    except Exception:
        return out


def _vector_features(feat: Dict[str, Any]) -> np.ndarray:
    keys = ["duracion_ms", "fc", "amp_rango", "amp_std", "asimetria_pico_valle", "b_rel", "c_rel", "x_rel", "pep_ms", "lvet_ms"]
    vals = []
    for k in keys:
        try:
            v = float(feat.get(k, np.nan))
        except Exception:
            v = np.nan
        vals.append(v)
...
def calcular_correccion_aprendida(features_actuales: Dict[str, Any], aprendizaje: Optional[Dict[str, Any]] = None) -> Dict[str, float]:
    """
    Devuelve deltas con signo aprendidos (final validado - automático).
    Usa primero casos morfológicamente cercanos; si son pocos, usa mediana robusta global.
    """
    aprendizaje = aprendizaje or st.session_state.get("aprendizaje_cursores", {}) or {}
    detalle = aprendizaje.get("historial_detallado") or []
    out = {"q": 0.0, "b": 0.0, "c": 0.0, "x": 0.0}
    if not detalle:
        return out

    actual = _vector_features(features_actuales)
    candidatos = []
    for r in detalle:
        feat = r.get("features_auto") or {}
        v = _vector_features(feat)
        ok = np.isfinite(actual) & np.isfinite(v)
        if ok.sum() >= 4:
            # normalización pragmática por magnitud para evitar que duración domine todo.
            scale = np.nanmedian(np.abs(np.vstack([actual[ok], v[ok]])), axis=0) + 1e-6
            dist = float(np.sqrt(np.nanmean(((actual[ok] - v[ok]) / scale) ** 2)))
        else:
            dist = 1e6
        candidatos.append((dist, r))

    candidatos = sorted(candidatos, key=lambda x: x[0])[:min(12, len(candidatos))]
    # Si hay suficientes vecinos razonables, ponderar por cercanía. Si no, usar historial global.
    usar = [r for d, r in candidatos if np.isfinite(d) and d < 2.5]
    if len(usar) < 3:
        usar = detalle[-min(30, len(detalle)):]

    n = int(aprendizaje.get("n", len(detalle)) or len(detalle))
    # Factor de aprendizaje progresivo: muy prudente al inicio, más fuerte con más validaciones.
    factor = min(0.80, max(0.10, n / 40.0))

    for cursor, campo in [("q", "delta_qrs_ms"), ("b", "delta_b_ms"), ("c", "delta_c_ms"), ("x", "delta_x_ms")]:
        vals = []
        pesos = []
        for idx, r in enumerate(usar):
            try:
                val = float(r.get(campo, np.nan))
                if np.isfinite(val):
                    vals.append(val)
                    pesos.append(1.0 + idx * 0.0)
            except Exception:
                pass
        if vals:
            # Mediana robusta: evita que una mala validación desplace todos los cursores.
            delta = float(np.nanmedian(vals)) * factor
            out[cursor] = float(np.clip(delta, -45.0, 45.0))
    return out


def aplicar_aprendizaje_a_puntos(df_curva: pd.DataFrame, puntos: Dict[str, Any], fc: Optional[float] = None) -> Dict[str, Any]:
    """Ajusta la propuesta automática con lo aprendido, sin perder control médico manual."""
    try:
        aprendizaje = st.session_state.get("aprendizaje_cursores") or cargar_aprendizaje_persistente()
        n = int(aprendizaje.get("n", 0) or 0)
        if n < 2:
            puntos["aprendizaje_aplicado"] = False
            puntos["aprendizaje_motivo"] = "Aprendizaje insuficiente: se requieren al menos 2 validaciones médicas."
            puntos["features_ml"] = features_curva_qbcx(df_curva, puntos, fc)
            return puntos
        feat = features_curva_qbcx(df_curva, puntos, fc)
        corr = calcular_correccion_aprendida(feat, aprendizaje)
        t = df_curva["tiempo_ms"].to_numpy(dtype=float)
        q_ms = float(puntos.get("q_ms", 0.0)) + corr.get("q", 0.0)
        b_ms = float(puntos.get("b_ms", 0.0)) + corr.get("b", 0.0)
        c_ms = float(puntos.get("c_ms", 0.0)) + corr.get("c", 0.0)
        x_ms = float(puntos.get("x_ms", 0.0)) + corr.get("x", 0.0)

        # Respetar orden fisiológico con separaciones mínimas.
        tmin, tmax = float(np.nanmin(t)), float(np.nanmax(t))
        q_ms = min(max(q_ms, tmin), tmax - 40)
        b_ms = max(b_ms, q_ms + 20)
        c_ms = max(c_ms, b_ms + 15)
        x_ms = max(x_ms, c_ms + 40)
        if x_ms > tmax:
            x_ms = tmax
            c_ms = min(c_ms, x_ms - 40)
            b_ms = min(b_ms, c_ms - 15)
            q_ms = min(q_ms, b_ms - 20)

        out = recalcular_puntos_manuales(df_curva, q_ms, b_ms, c_ms, x_ms)
        out["aprendizaje_aplicado"] = True
        out["aprendizaje_n_validaciones"] = n
        out["correccion_aprendida_q_ms"] = corr.get("q", 0.0)
        out["correccion_aprendida_b_ms"] = corr.get("b", 0.0)
        out["correccion_aprendida_c_ms"] = corr.get("c", 0.0)
        out["correccion_aprendida_x_ms"] = corr.get("x", 0.0)
        out["features_ml"] = feat
        out["confianza"] = (out.get("confianza") or "") + f" + aprendizaje supervisado n={n}"
        out["alertas"] = list(out.get("alertas", []))
        out["alertas"].append("Cursores ajustados por aprendizaje supervisado; confirmar visualmente antes de validar.")
        return out
    except Exception:
        puntos["aprendizaje_aplicado"] = False
        puntos["features_ml"] = features_curva_qbcx(df_curva, puntos, fc)
...
def actualizar_aprendizaje_cursores(pauto: Dict[str, Any], pfinal: Dict[str, Any], validado: bool) -> Dict[str, Any]:
    ...
    if validado:
        ...
        n_validacion = int(estado.get("n", 0))
        registro_delta = {
            "validacion_n": n_validacion,
            "fecha_hora": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "features_auto": pauto.get("features_ml", {}),
            "hash_curva": pauto.get("hash_curva", ""),
            "aprendizaje_aplicado_en_auto": bool(pauto.get("aprendizaje_aplicado", False)),
            "aprendizaje_n_previo": int(pauto.get("aprendizaje_n_validaciones", 0) or 0),
            "qrs_auto_ms": float(pauto.get("q_ms", np.nan)),
            "qrs_final_ms": float(pfinal.get("q_ms", np.nan)),
            "delta_qrs_ms": float(pfinal.get("q_ms", np.nan)) - float(pauto.get("q_ms", np.nan)),
            "abs_delta_qrs_ms": float(diffs_por_cursor.get("q", np.nan)),
            "b_auto_ms": float(pauto.get("b_ms", np.nan)),
            "b_final_ms": float(pfinal.get("b_ms", np.nan)),
            "delta_b_ms": float(pfinal.get("b_ms", np.nan)) - float(pauto.get("b_ms", np.nan)),
            "abs_delta_b_ms": float(diffs_por_cursor.get("b", np.nan)),
            "c_auto_ms": float(pauto.get("c_ms", np.nan)),
            "c_final_ms": float(pfinal.get("c_ms", np.nan)),
            "delta_c_ms": float(pfinal.get("c_ms", np.nan)) - float(pauto.get("c_ms", np.nan)),
            "abs_delta_c_ms": float(diffs_por_cursor.get("c", np.nan)),
            "x_auto_ms": float(pauto.get("x_ms", np.nan)),
            "x_final_ms": float(pfinal.get("x_ms", np.nan)),
            "delta_x_ms": float(pfinal.get("x_ms", np.nan)) - float(pauto.get("x_ms", np.nan)),
            "abs_delta_x_ms": float(diffs_por_cursor.get("x", np.nan)),
            "correccion_media_abs_ms": err,
            "correccion_max_abs_ms": float(np.nanmax(list(diffs_por_cursor.values()))) if diffs_por_cursor else np.nan,
            "pep_auto_ms": float(pauto.get("pep_ms", np.nan)),
            "pep_final_ms": float(pfinal.get("pep_ms", np.nan)),
            "delta_pep_ms": float(pfinal.get("pep_ms", np.nan)) - float(pauto.get("pep_ms", np.nan)),
            "lvet_auto_ms": float(pauto.get("lvet_ms", np.nan)),
            "lvet_final_ms": float(pfinal.get("lvet_ms", np.nan)),
            "delta_lvet_ms": float(pfinal.get("lvet_ms", np.nan)) - float(pauto.get("lvet_ms", np.nan)),
        }
        estado.setdefault("historial_detallado", []).append(registro_delta)
        estado["ultimo_error_ms"] = err
        estado["ultimo_error_por_cursor"] = diffs_por_cursor
        estado["ultima_actualizacion"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        st.session_state["aprendizaje_cursores"] = estado
        guardar_aprendizaje_persistente(estado)
    return estado
...
def detectar_puntos_qbcx_icg(df_curva: pd.DataFrame, fc: Optional[float] = None) -> Dict[str, Any]:
    ...
    # Aplicación gradual de correcciones aprendidas por validaciones médicas previas.
    res = aplicar_aprendizaje_a_puntos(df_curva, res, fc)
    return res
...
def dataframe_delta_aprendizaje_cursores(aprendizaje: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
    """Devuelve una tabla longitudinal con los deltas entre cursores automáticos y cursores finales validados.

    Cada fila representa una validación médica. Los deltas con signo indican hacia dónde corrigió
    el operador; los deltas absolutos permiten seguir la reducción del error como evolución del aprendizaje.
    """
    aprendizaje = aprendizaje or st.session_state.get("aprendizaje_cursores", {}) or {}
    detalle = aprendizaje.get("historial_detallado") or []
    if detalle:
    ...
    # Compatibilidad con versiones previas que solo guardaban listas por cursor.
    hist = aprendizaje.get("historial_error_ms") or []
    qs = aprendizaje.get("historial_q_ms") or []
    bs = aprendizaje.get("historial_b_ms") or []
    cs = aprendizaje.get("historial_c_ms") or []
    xs = aprendizaje.get("historial_x_ms") or []
    filas = []
    for i, err in enumerate(hist):
        filas.append({
            "validacion_n": i + 1,
            "fecha_hora": "",
            "abs_delta_qrs_ms": qs[i] if i < len(qs) else np.nan,
            "abs_delta_b_ms": bs[i] if i < len(bs) else np.nan,
            "abs_delta_c_ms": cs[i] if i < len(cs) else np.nan,
            "abs_delta_x_ms": xs[i] if i < len(xs) else np.nan,
            "correccion_media_abs_ms": err,
        })
    return pd.DataFrame(filas)


def excel_aprendizaje_cursores_bytes(aprendizaje: Optional[Dict[str, Any]] = None) -> bytes:
    """Exporta a Excel la evolución del aprendizaje de cursores QRS-B-C-X."""
    aprendizaje = aprendizaje or st.session_state.get("aprendizaje_cursores", {}) or {}
    df_delta = dataframe_delta_aprendizaje_cursores(aprendizaje)
    resumen = {
        "validaciones_totales": int(aprendizaje.get("n", 0) or 0),
        "correccion_media_historica_ms": aprendizaje.get("error_ms_prom"),
        "ultima_correccion_media_ms": aprendizaje.get("ultimo_error_ms"),
    }
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        if not df_delta.empty:
            df_delta.to_excel(writer, sheet_name="Delta_cursores", index=False)
        else:
            pd.DataFrame([{"Estado": "Sin validaciones registradas"}]).to_excel(writer, sheet_name="Delta_cursores", index=False)
        pd.DataFrame([resumen]).to_excel(writer, sheet_name="Resumen_aprendizaje", index=False)
        pd.DataFrame({
            "Campo": [
                "delta_*_ms",
                "abs_delta_*_ms",
                "correccion_media_abs_ms",
                "correccion_max_abs_ms",
                "delta_pep_ms / delta_lvet_ms",
            ],
            "Interpretacion": [
                "Diferencia con signo: cursor final validado por el médico menos cursor automático propuesto por la app.",
                "Diferencia absoluta en milisegundos por cursor; mide magnitud de corrección sin dirección.",
                "Promedio de los errores absolutos de QRS, B, C y X en esa validación.",
                "Mayor corrección absoluta entre QRS, B, C y X en esa validación.",
                "Cambio secundario en tiempos sistólicos derivado de la corrección manual.",
            ]
...
def excel_metricas_hemodinamicas_bytes(
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        if df_metricas is not None and not df_metricas.empty:
            df_metricas.to_excel(writer, sheet_name="Metricas_hemodinamicas", index=False)
        else:
        ...
        if aprendizaje:
            df_delta_ap = dataframe_delta_aprendizaje_cursores(aprendizaje)
            if not df_delta_ap.empty:
                df_delta_ap.to_excel(writer, sheet_name="Delta_aprendizaje", index=False)
            resumen_ap = {
                "validaciones_totales": int(aprendizaje.get("n", 0) or 0),
                "correccion_media_historica_ms": aprendizaje.get("error_ms_prom"),
                "ultima_correccion_media_ms": aprendizaje.get("ultimo_error_ms"),
            }
            pd.DataFrame([resumen_ap]).to_excel(writer, sheet_name="Aprendizaje_cursores", index=False)
        pd.DataFrame({
            "Contenido": [
...
def filtrar_metricas_pdf_morfologicas(df_der: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    permitidas = [
        ...
        "FE Capan",
        "IV importado - indice de velocidad", "IH importado - indice de Heather",
        "IAC importado - indice de aceleracion", "CTS/STR importado",
        "Indice de velocidad relativo", "Indice de Heather relativo",
...
def _clasificar_variable_contractilidad(nombre: str, valor: Optional[float]) -> Optional[str]:
    """Clasificación por rangos Z-Logic para integrar el dominio contractilidad.

    Se usa solo cuando la variable tiene rango de referencia operativo conocido.
    Las variables morfológicas relativas se muestran como apoyo, pero no se usan
    para clasificar disminuida/conservada/aumentada si no tienen calibración absoluta.
    """
    v = limpiar_numero(valor)
    if v is None:
        return None
    ...
    if "iac" in n or "aceleracion" in n or "aceleración" in n:
        if v < 70:
            return "disminuida"
        if v > 150:
            return "aumentada"
    ...
    if "cts" in n or "str" in n or "pep/lvet" in n or "relacion" in n:
        # En Z-Logic el CTS suele informarse como fracción o porcentaje.
        r = v / 100.0 if v > 2 else v
        if r > 0.50:
            return "disminuida"
        if r < 0.30:
            return "aumentada"
    ...
    # IH/Heather no se fuerza a clasificación absoluta si proviene de escala relativa.
    return None


def clasificar_contractilidad_integrada(
    iv: Optional[float],
    ih: Optional[float],
    iac: Optional[float],
    itc: Optional[float],
    rts: Optional[float],
    usa_metricas_relativas: bool = False,
) -> str:
    """Conclusión integrada del dominio contractilidad.

    Integra IV, IH, IAC, ITC y relación de tiempos sistólicos. La decisión
    disminuida/conservada/aumentada se apoya en variables con rangos Z-Logic
    conocidos: IV, IAC, ITC y CTS/STR. IH se incorpora como variable de apoyo
    cuando está disponible, pero no define por sí sola el estado si la escala es
    relativa.
    """
    votos = []
    for nombre, valor in [
        ...
        c = _clasificar_variable_contractilidad(nombre, valor)
        if c:
            votos.append(c)

    if not votos:
        if ih is not None:
            return "Contractilidad no clasificable con rangos absolutos; IH disponible como apoyo morfológico."
        return "No clasificable"

    n_dis = votos.count("disminuida")
    n_con = votos.count("conservada")
    n_aum = votos.count("aumentada")

    if n_dis >= 2 or (n_dis == 1 and n_aum == 0 and n_con == 0):
        estado = "Contractilidad disminuida"
    elif n_aum >= 2 or (n_aum == 1 and n_dis == 0 and n_con == 0):
        estado = "Contractilidad aumentada"
    elif n_con >= max(n_dis, n_aum):
        estado = "Contractilidad conservada"
    else:
        estado = "Contractilidad mixta / discordante"

    detalles = []
    if iv is not None:
        detalles.append(f"IV {_clasificar_variable_contractilidad('IV', iv) or 'de apoyo'}")
    if ih is not None:
        detalles.append("IH disponible como apoyo")
    if iac is not None:
        detalles.append(f"IAC {_clasificar_variable_contractilidad('IAC', iac) or 'de apoyo'}")
    if itc is not None:
        detalles.append(f"ITC {_clasificar_variable_contractilidad('ITC', itc) or 'de apoyo'}")
    if rts is not None:
        detalles.append(f"relación PEP/LVET {_clasificar_variable_contractilidad('CTS/STR', rts) or 'de apoyo'}")

    if usa_metricas_relativas:
        detalles.append("métricas morfológicas relativas interpretadas con cautela por ausencia de calibración vertical absoluta")

    return estado + (" (" + "; ".join(detalles) + ")." if detalles else ".")
...
def construir_dominios_hemodinamicos(df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Para contractilidad se priorizan valores importados/calibrados del equipo
    # cuando existen; si no están disponibles, se usan métricas morfológicas
    # relativas como apoyo interpretativo.
    iv_abs = _mv(df_der, "IV importado - indice de velocidad")
    ih_abs = _mv(df_der, "IH importado - indice de Heather")
    iac_abs = _mv(df_der, "IAC importado - indice de aceleracion")
    cts_abs = _mv(df_der, "CTS/STR importado")

    iv_rel = _mv(df_der, "Indice de velocidad relativo")
    ih_rel = _mv(df_der, "Indice de Heather relativo")
    iac_rel = _mv(df_der, "Indice de aceleracion relativo")

    iv = iv_abs if iv_abs is not None else iv_rel
    ih = ih_abs if ih_abs is not None else ih_rel
    iac = iac_abs if iac_abs is not None else iac_rel
    itc = _mv(df_der, "ITC aproximado")
    rts = cts_abs if cts_abs is not None else _mv(df_der, "STR/CTS morfologico")

    usa_rel = any(v is not None for v in [iv_rel, ih_rel, iac_rel]) and all(v is None for v in [iv_abs, ih_abs, iac_abs])
    ac = _mv(df_der, "Acoplamiento VA Ea/Ees")
    cft = _mv(df_der, "CFT/TFC")
    contractilidad = clasificar_contractilidad_integrada(iv, ih, iac, itc, rts, usa_metricas_relativas=usa_rel)
...
def tabla_dominios_hemodinamicos(df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        ...
        {"Dominio": "Contractilidad", "Valores": f"IV {fmt_metrica('IV', d['iv'])}; IH {fmt_metrica('IH', d['ih'])}; IAC {fmt_metrica('IAC', d['iac'])}; ITC {fmt_metrica('ITC', d['itc'])}; PEP/LVET {fmt_metrica('CTS', d['rts'])}", "Interpretación": d["contractilidad"]},
...
def diagnostico_hemodinamico_desde_morfologia(df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> str:
    ...
    return " ".join([
        ...
        f"Contractilidad: {d['contractilidad']}",
        f"Rendimiento cardiovascular: {d['rendimiento']} según acoplamiento ventrículo-arterial.",
        f"Volemia: {d['volemia']} por CFT/TFC.",
    ])


def _valor_metricas_comparacion(df_der: Optional[pd.DataFrame], *nombres: str) -> Optional[float]:
    """Busca una metrica por coincidencia flexible para comparar automatico vs validado."""
    if df_der is None or getattr(df_der, "empty", True):
        return None
    try:
        col_var = "Variable" if "Variable" in df_der.columns else df_der.columns[0]
        for nombre in nombres:
            mask = df_der[col_var].astype(str).str.lower().str.contains(str(nombre).lower(), na=False, regex=False)
            if mask.any():
                return limpiar_numero(df_der.loc[mask].iloc[0].get("Valor recalculado"))
    except Exception:
        return None
    return None


def _categoria_contractilidad_resumida(txt: Any) -> str:
    n = normalizar_txt(txt)
    if "dismin" in n or "lento" in n:
        return "Contractilidad disminuida"
    if "aument" in n or "rapido" in n:
        return "Contractilidad aumentada"
    if "conserv" in n or "intermedio" in n or "balance" in n:
        return "Contractilidad conservada/intermedia"
    if "no clasific" in n:
        return "No clasificable"
    return str(txt or "No clasificable")[:120]


def construir_comparacion_clinica_auto_validado(
    df_base: pd.DataFrame,
    morfo_auto: Optional[Dict[str, Any]],
    morfo_validado: Optional[Dict[str, Any]],
) -> Dict[str, Any]:
    """Compara el informe automatico contra el corregido/validado por el medico.

    El objetivo no es solo cuantificar deltas de cursores, sino determinar si la correccion
    cambia dominios o patrones hemodinamicos con impacto clinico.
    """
    if morfo_auto is None or morfo_validado is None:
        return {"disponible": False, "conclusion": "Comparación no disponible: faltan cursores automáticos o cursores corregidos/validados."}
    basal_cmp, _ = seleccionar_basal_y_parado(df_base)
    datos_cmp = basal_cmp or {}
    p_auto = dict(morfo_auto.get("puntos") or {})
    p_val = dict(morfo_validado.get("puntos") or {})
    if not p_auto or not p_val:
        return {"disponible": False, "conclusion": "Comparación no disponible: faltan puntos QRS-B-C-X."}

    df_auto = filtrar_metricas_pdf_morfologicas(calcular_variables_icg_derivadas(datos_cmp, morfo_auto))
    df_val = filtrar_metricas_pdf_morfologicas(calcular_variables_icg_derivadas(datos_cmp, morfo_validado))
    dom_auto = construir_dominios_hemodinamicos(df_auto, p_auto)
    dom_val = construir_dominios_hemodinamicos(df_val, p_val)

    filas: List[Dict[str, Any]] = []
    cambios_relevantes = 0
    cambios_moderados = 0

    dominios = [
        ("Flujo por IC", "flujo_ic"),
        ("Flujo por IDS", "flujo_ids"),
        ("Resistencia vascular", "flujo_irv"),
        ("Función cardíaca", "funcion"),
        ("Contractilidad", "contractilidad"),
        ("Rendimiento / acoplamiento VA", "rendimiento"),
        ("Volemia", "volemia"),
    ]
    for dominio, clave in dominios:
        a = dom_auto.get(clave)
        v = dom_val.get(clave)
        if clave == "contractilidad":
            a_cmp = _categoria_contractilidad_resumida(a)
            v_cmp = _categoria_contractilidad_resumida(v)
        else:
            a_cmp, v_cmp = str(a), str(v)
        cambia = a_cmp != v_cmp
        impacto = "Sin cambio clínico de categoría"
        if cambia:
            cambios_relevantes += 1
            impacto = "CAMBIA el patrón/diagnóstico del dominio"
        filas.append({
            "Dominio": dominio,
            "Automático": a_cmp,
            "Corregido por médico": v_cmp,
            "Cambio de patrón": "Sí" if cambia else "No",
            "Relevancia clínica": impacto,
        })

    metricas = [
        ("QRS", p_auto.get("q_ms"), p_val.get("q_ms"), 10.0, "ms"),
        ("B", p_auto.get("b_ms"), p_val.get("b_ms"), 10.0, "ms"),
        ("C", p_auto.get("c_ms"), p_val.get("c_ms"), 10.0, "ms"),
        ("X", p_auto.get("x_ms"), p_val.get("x_ms"), 15.0, "ms"),
        ("PEP", p_auto.get("pep_ms"), p_val.get("pep_ms"), 20.0, "ms"),
        ("LVET", p_auto.get("lvet_ms"), p_val.get("lvet_ms"), 30.0, "ms"),
        ("FE Capan", _valor_metricas_comparacion(df_auto, "FE Capan"), _valor_metricas_comparacion(df_val, "FE Capan"), 0.05, "fracción"),
        ("STR/CTS", _valor_metricas_comparacion(df_auto, "STR/CTS"), _valor_metricas_comparacion(df_val, "STR/CTS"), 0.05, "ratio"),
        ("Ea", _valor_metricas_comparacion(df_auto, "Ea"), _valor_metricas_comparacion(df_val, "Ea"), 0.15, "mmHg/ml"),
        ("Ees", _valor_metricas_comparacion(df_auto, "Ees"), _valor_metricas_comparacion(df_val, "Ees"), 0.15, "mmHg/ml"),
        ("Acoplamiento VA", _valor_metricas_comparacion(df_auto, "Acoplamiento VA"), _valor_metricas_comparacion(df_val, "Acoplamiento VA"), 0.10, "ratio"),
    ]
    filas_metricas: List[Dict[str, Any]] = []
    deltas_cursor = []
    for nombre, a, v, umbral, unidad in metricas:
        av = limpiar_numero(a)
        vv = limpiar_numero(v)
        delta = None if av is None or vv is None else vv - av
        abs_delta = None if delta is None else abs(delta)
        supera = bool(abs_delta is not None and abs_delta >= umbral)
        if supera and nombre in {"PEP", "LVET", "FE Capan", "STR/CTS", "Ea", "Ees", "Acoplamiento VA"}:
            cambios_moderados += 1
        if supera and nombre in {"QRS", "B", "C", "X"}:
            deltas_cursor.append(abs_delta)
        filas_metricas.append({
            "Métrica": nombre,
            "Automático": av,
            "Corregido por médico": vv,
            "Delta médico - automático": delta,
            "Unidad": unidad,
            "Umbral de alerta": umbral,
            "Supera umbral": "Sí" if supera else "No",
        })

    if cambios_relevantes > 0:
        nivel = "ALTA"
        conclusion = f"Las correcciones manuales tienen relevancia clínica alta: modifican {cambios_relevantes} dominio(s) hemodinámico(s). Se recomienda usar el PDF corregido por médico como informe final y conservar el caso para entrenamiento supervisado."
    elif cambios_moderados > 0:
        nivel = "MODERADA"
        conclusion = f"Las correcciones no cambian el patrón categórico, pero modifican métricas derivadas por encima de umbrales operativos en {cambios_moderados} variable(s). Tienen relevancia clínica moderada y deben alimentar el aprendizaje."
    else:
        nivel = "BAJA"
        conclusion = "Las correcciones no modifican dominios hemodinámicos ni métricas derivadas por encima de los umbrales operativos. La relevancia clínica de la corrección es baja para este caso."

    return {
        "disponible": True,
        "nivel_relevancia": nivel,
        "conclusion": conclusion,
        "df_patrones": pd.DataFrame(filas),
        "df_metricas": pd.DataFrame(filas_metricas),
        "dominios_auto": dom_auto,
        "dominios_validado": dom_val,
        "diagnostico_auto": diagnostico_hemodinamico_desde_morfologia(df_auto, p_auto),
        "diagnostico_validado": diagnostico_hemodinamico_desde_morfologia(df_val, p_val),
        "deltas_cursor_media_abs_ms": float(np.nanmean(deltas_cursor)) if deltas_cursor else 0.0,
    }


def excel_comparacion_clinica_bytes(comparacion: Dict[str, Any]) -> bytes:
    """Exporta el análisis de relevancia clínica automático vs corregido."""
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        pd.DataFrame([{
            "relevancia_clinica": comparacion.get("nivel_relevancia", "No disponible"),
            "conclusion": comparacion.get("conclusion", "No disponible"),
            "diagnostico_automatico": comparacion.get("diagnostico_auto", ""),
            "diagnostico_corregido": comparacion.get("diagnostico_validado", ""),
        }]).to_excel(writer, sheet_name="Resumen_clinico", index=False)
        dfp = comparacion.get("df_patrones")
        if isinstance(dfp, pd.DataFrame) and not dfp.empty:
            dfp.to_excel(writer, sheet_name="Cambio_patrones", index=False)
        dfm = comparacion.get("df_metricas")
        if isinstance(dfm, pd.DataFrame) and not dfm.empty:
            dfm.to_excel(writer, sheet_name="Delta_metricas", index=False)
        pd.DataFrame({
            "Criterio": ["Alta", "Moderada", "Baja"],
            "Definicion": [
                "Cambia al menos un dominio hemodinamico categórico entre automático y corregido.",
                "No cambia dominio, pero PEP/LVET/FE/STR/Ea/Ees/AC superan umbrales operativos.",
                "No cambia dominio ni métricas derivadas por encima de umbrales operativos.",
            ]
...
def generar_pdf_informe(df: pd.DataFrame, morfo: Optional[Dict[str, Any]] = None) -> bytes:
    """PDF de cardiografía por impedancia.

    Puede generarse en dos modos:
    - automático: usa cursores propuestos por la app y ajustados por aprendizaje supervisado previo;
    - validado: usa cursores corregidos/confirmados por el médico.
    """
    if A4 is None:
    ...
    automatico = bool(morfo.get("automatico", False))
    titulo_pdf = morfo.get(
        "titulo_pdf",
        "Informe automático de cardiografía por impedancia" if automatico else "Análisis morfológico de onda de impedancia dZ/dt con cursores QRS-B-C-X",
    )
    # Reemplazar el título inicial por el título específico del modo de informe.
    elems[0] = Paragraph(safe_pdf_text(titulo_pdf), styles["TituloAzul"])
    estado_pdf = "Automático con cursores propuestos/aprendidos" if automatico else ("Validado por médico" if validado else "Generado con cursores no validados")
    base_pdf = "Onda dZ/dt con QRS-B-C-X automático" if automatico else "Onda dZ/dt con QRS-B-C-X"
    tabla_datos = [["Paciente", safe_pdf_text(paciente), "Estado", safe_pdf_text(estado_pdf)], ["Base analítica", safe_pdf_text(base_pdf), "Archivo", safe_pdf_text(datos.get("archivo_origen", ""))]]
    ...
    if morfo.get("automatico", False):
        elems.append(Paragraph(
            "Informe generado en modo automático: la app colocó los cursores mediante reglas fisiológicas, extracción de señal y correcciones aprendidas de validaciones previas. La revisión médica sigue siendo recomendable cuando la confianza sea intermedia o baja.",
            styles["Advertencia"]
        ))
    ...
    # Conclusión específica del dominio contractilidad, separada de la tabla.
    try:
        dom_ct = construir_dominios_hemodinamicos(df_der_pdf, puntos)
        elems.append(Paragraph("Conclusión del dominio contractilidad", styles["Sub"]))
        elems.append(Paragraph(safe_pdf_text(dom_ct.get("contractilidad", "No clasificable")), styles["NormalSmall"]))
        elems.append(Spacer(1, 6))
    except Exception:
        pass
    ...
    comparacion_pdf = morfo.get("comparacion_clinica") if isinstance(morfo, dict) else None
    if comparacion_pdf and comparacion_pdf.get("disponible"):
        elems.append(Paragraph("Comparación clínica: automático vs corregido por médico", styles["Sub"]))
        elems.append(Paragraph(safe_pdf_text(comparacion_pdf.get("conclusion", "")), styles["Advertencia"]))
        df_cmp_pat = comparacion_pdf.get("df_patrones")
        if isinstance(df_cmp_pat, pd.DataFrame) and not df_cmp_pat.empty:
            data_cmp = [df_cmp_pat.columns.tolist()] + df_cmp_pat.astype(str).values.tolist()
            data_cmp = [[safe_pdf_text(c) for c in row] for row in data_cmp]
            tcmp = Table(data_cmp, colWidths=[3.2*cm, 4.0*cm, 4.0*cm, 2.0*cm, 4.0*cm], repeatRows=1)
            tcmp.setStyle(TableStyle([("BACKGROUND", (0,0), (-1,0), colors.HexColor("#7C2D12")), ("TEXTCOLOR", (0,0), (-1,0), colors.white), ("GRID", (0,0), (-1,-1), .3, colors.HexColor("#D7E3EE")), ("VALIGN", (0,0), (-1,-1), "TOP"), ("FONTSIZE", (0,0), (-1,-1), 6.2)]))
            elems.append(tcmp)
            elems.append(Spacer(1, 8))
...
inicializar_aprendizaje_en_session()
...
if "df_curva_icg" in st.session_state and "puntos_qbcx_auto" in st.session_state:
    ...
    pauto.setdefault("hash_curva", hash_curva_operativo(df_curva))
    pauto.setdefault("features_ml", features_curva_qbcx(df_curva, pauto, fc_det))
    ...
    with col_val2:
        if st.button("Reiniciar contador de aprendizaje", key="btn_reiniciar_aprendizaje_qbcx"):
            st.session_state["aprendizaje_cursores"] = estado_aprendizaje_vacio()
            guardar_aprendizaje_persistente(st.session_state["aprendizaje_cursores"])
            st.session_state["qbcx_validado_operador"] = False
    ...
    cols_dl[5].download_button(
        "Exportar aprendizaje cursores Excel",
        data=excel_aprendizaje_cursores_bytes(aprendizaje_export),
        file_name="aprendizaje_delta_cursores_qbcx.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
...
def construir_morfo_payload_para_pdf(modo: str = "validado") -> Optional[Dict[str, Any]]:
    """Arma el payload del PDF automático o validado sin exigir interacción adicional."""
    if "df_curva_icg" not in st.session_state:
        return None
    df_curva_loc = st.session_state.get("df_curva_icg")
    if df_curva_loc is None or getattr(df_curva_loc, "empty", True):
        return None

    if modo == "automatico":
        puntos_base = st.session_state.get("puntos_qbcx_auto") or st.session_state.get("puntos_qbcx_final")
        if not puntos_base:
            return None
        puntos_pdf = dict(puntos_base)
        puntos_pdf["validado_operador"] = False
        puntos_pdf["modo_informe"] = "automatico_con_aprendizaje_supervisado"
        titulo = "Informe automático de cardiografía por impedancia"
        automatico = True
        interpretacion = texto_interpretacion_qbcx(puntos_pdf)
    else:
        puntos_base = st.session_state.get("puntos_qbcx_final") or st.session_state.get("puntos_qbcx_auto")
        if not puntos_base:
            return None
        puntos_pdf = dict(puntos_base)
        titulo = "Informe de cardiografía por impedancia con análisis morfológico validado"
        automatico = False
        interpretacion = st.session_state.get("interpretacion_qbcx", texto_interpretacion_qbcx(puntos_pdf))

    png_qbcx_local = None
    try:
        fig_pdf = graficar_curva_icg_con_puntos(df_curva_loc, puntos_pdf)
        png_qbcx_local = fig_to_png_bytes(fig_pdf)
        plt.close(fig_pdf)
    except Exception:
        png_qbcx_local = st.session_state.get("qbcx_png")

    png_canales_local = None
    if "canales_zlogic_pdf" in st.session_state:
        try:
            fig_canales_pdf = graficar_canales_zlogic(
                st.session_state["canales_zlogic_pdf"],
                puntos_pdf,
                amplificar_fono=float(st.session_state.get("amplificar_fono_visual", 4.00)),
                amplificar_ecg=float(st.session_state.get("amplificar_ecg_visual", 1.80)),
                expandir_vertical=True,
            )
            png_canales_local = fig_to_png_bytes(fig_canales_pdf)
            plt.close(fig_canales_pdf)
        except Exception:
            png_canales_local = st.session_state.get("canales_zlogic_png")

    return {
        "df_curva": df_curva_loc,
        "puntos": puntos_pdf,
        "interpretacion": interpretacion,
        "png": png_qbcx_local,
        "png_canales": png_canales_local,
        "automatico": automatico,
        "titulo_pdf": titulo,
    }

st.markdown("<div class='card'>", unsafe_allow_html=True)
st.subheader("6. Exportación de informes PDF y Excel")
if not df_final.empty:
    basal, _ = seleccionar_basal_y_parado(df_final)
    nombre_paciente = limpiar_nombre_archivo((basal or {}).get("paciente", "PACIENTE"))

    morfo_auto_payload = construir_morfo_payload_para_pdf("automatico")
    if morfo_auto_payload is not None:
        try:
            pdf_auto = generar_pdf_informe(df_final, morfo_auto_payload)
            st.download_button(
                "Descargar PDF automático de cardiografía por impedancia",
                data=pdf_auto,
                file_name=f"{nombre_paciente} CARDIOGRAFIA IMPEDANCIA AUTOMATICA.pdf",
                mime="application/pdf",
                help="Generado con cursores automáticos ajustados por aprendizaje supervisado previo. No requiere registrar validación manual.",
            )
        except Exception as e:
            st.error(f"PDF automático no disponible. Error real: {e}")
    else:
        st.info("Para generar el PDF automático completo, primero digitalizar automáticamente el PDF Z-Logic o una captura de la curva dZ/dt.")

    morfo_validado_payload = construir_morfo_payload_para_pdf("validado")
    if morfo_validado_payload is not None:
        try:
            pdf_validado = generar_pdf_informe(df_final, morfo_validado_payload)
            st.download_button(
                "Descargar PDF morfológico validado",
                data=pdf_validado,
                file_name=f"{nombre_paciente} ANALISIS MORFOLOGICO ICG VALIDADO.pdf",
                mime="application/pdf",
            )
        except Exception as e:
            st.error(f"PDF validado no disponible. Error real: {e}")

    # Comparación de relevancia clínica entre PDF automático y PDF corregido por médico.
    if morfo_auto_payload is not None and morfo_validado_payload is not None:
        try:
            comparacion_clinica = construir_comparacion_clinica_auto_validado(df_final, morfo_auto_payload, morfo_validado_payload)
            st.session_state["comparacion_clinica_auto_validado"] = comparacion_clinica
            morfo_auto_payload["comparacion_clinica"] = comparacion_clinica
            morfo_validado_payload["comparacion_clinica"] = comparacion_clinica
            st.markdown("### Relevancia clínica de la corrección médica")
            if comparacion_clinica.get("disponible"):
                nivel = comparacion_clinica.get("nivel_relevancia", "No disponible")
                if nivel == "ALTA":
                    st.error(comparacion_clinica.get("conclusion"))
                elif nivel == "MODERADA":
                    st.warning(comparacion_clinica.get("conclusion"))
                else:
                    st.success(comparacion_clinica.get("conclusion"))
                df_pat_cmp = comparacion_clinica.get("df_patrones")
                if isinstance(df_pat_cmp, pd.DataFrame) and not df_pat_cmp.empty:
                    st.dataframe(df_pat_cmp, use_container_width=True)
                df_met_cmp = comparacion_clinica.get("df_metricas")
                if isinstance(df_met_cmp, pd.DataFrame) and not df_met_cmp.empty:
                    with st.expander("Ver deltas de cursores y métricas derivadas"):
                        st.dataframe(df_met_cmp, use_container_width=True)
                excel_cmp = excel_comparacion_clinica_bytes(comparacion_clinica)
                st.download_button(
                    "Descargar Excel comparativo de relevancia clínica",
                    data=excel_cmp,
                    file_name=f"{nombre_paciente} comparacion clinica automatico vs corregido.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                )
                try:
                    pdf_auto_cmp = generar_pdf_informe(df_final, morfo_auto_payload)
                    st.download_button(
                        "Descargar PDF automático con comparación clínica",
                        data=pdf_auto_cmp,
                        file_name=f"{nombre_paciente} CARDIOGRAFIA AUTOMATICA CON COMPARACION.pdf",
                        mime="application/pdf",
                    )
                    pdf_val_cmp = generar_pdf_informe(df_final, morfo_validado_payload)
                    st.download_button(
                        "Descargar PDF corregido con comparación clínica",
                        data=pdf_val_cmp,
                        file_name=f"{nombre_paciente} CARDIOGRAFIA CORREGIDA CON COMPARACION.pdf",
                        mime="application/pdf",
                    )
                except Exception as e_pdf_cmp:
                    st.warning(f"Comparación visible, pero no se pudo incrustar en PDF: {e_pdf_cmp}")
            else:
                st.info(comparacion_clinica.get("conclusion"))
        except Exception as e:
            st.error(f"No se pudo construir la comparación clínica automático vs corregido: {e}")
    ...
    if "df_metricas_hemodinamicas_digitalizacion" in st.session_state:
        ...
        st.download_button(
            "Descargar Excel con todas las métricas hemodinámicas y deltas de aprendizaje",
            data=excel_final,
            file_name=f"{nombre_paciente} metricas hemodinamicas ICG.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )
else:
    st.info("Para exportar PDF completo, primero cargar datos CGI/Z-Logic. La curva morfológica se puede analizar y descargar por separado.")
st.markdown("</div>", unsafe_allow_html=True)
